...
Take last week's `This week` items. Anything no source confirms as done becomes
an "Owed from `<date>`" line this week. If the same item was already carried
last week too, say how many weeks running — that count is what makes a stale
item visible instead of quietly re-listed forever.
...
items you've flagged to raise. Surface unraised ones under `Process item to
raise` — this skill reads that note, it doesn't maintain it.
...
Drop a section entirely rather than leaving it empty. `Process item to raise`
only appears when `standup-agenda.md` has something pending.
