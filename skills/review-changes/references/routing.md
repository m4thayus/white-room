...
This file owns the destination tables for a Precedent observation and for a
Prior Round disposition.

**Use what Precedent reported.** It decides who owns a fix, and what a finding
covers. It never decides whether a finding is real, or how severe it is.
...
A bug main also has is the same bug. Prior art changes what the comment says,
not what it asks for. Say that the pattern predates this diff, and that the fix
reaches past it.

Ask what a finding actually is before you reach for prior art. A finding
rewritten around the points that survive it is frequently the wrong finding.

**Route the Prior Round rows.** The axis reports what it saw, and it does not
...
**Ignored in silence.** How hard the prior comment asked decides. A non-blocking
ask left the change optional, so re-raising it removes the option.
