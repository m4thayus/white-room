export const meta = {
  name: "review",
  description:
    "Dispatch the review-changes axes as one pipeline, then sweep history behind the axes that rule on what the repo already does",
  whenToUse:
    "Step 2 of the white-room:review-changes skill. args carry {skillDir, diff, commits, axes, sibling}, where each axis entry is {axis, payload, model, effort}. Returns every axis report verbatim, and posts nothing.",
  phases: [
...
if (!spec.skillDir || !spec.diff || !axes.length) {
  return {
    error:
      "This workflow needs args {skillDir, diff, commits, axes:[{axis}]}. Nothing ran, so dispatch the axes by hand."
  }
}
